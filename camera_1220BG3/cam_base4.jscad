...
base = (function() {
  ...
  function base() {
    ...
    dim.base.x = 4 * dim.gnd.furo + 3 * dim.gnd.espaco_entre_furos + 2* dim.base.parede_grossa
    ...
    var suporte_i1 = cube({
      size: [interno_x, dim.suporte.y, dim.slit.y + 2*dim.base.folga_slit],
      radius: [dim.slit.x, 0, 2],
      center: [1, 0, 0],
      fn: dim.fn
    }).translate([0, 0, dim.y/2 - dim.slit.y/2 - dim.base.folga_slit + 10])
    ...
    var suporte_i3 = cube({
      size: [dim.x + dim.base.folga_slit, dim.suporte.y, dim.suporte.z],
      center: [1,0,0]
    }).translate([0, 0, dim.base.z + 4* dim.base.parede_grossa]).setColor([1,0,0])
    ...
    .rotateY(270)
    .translate([dim.base.x/2, dim.base.parede_grossa, dim.y/2 - dim.slit.raio_parafuso + 10])
  ...
  function full() {
    ...
    return base().union(
      ...
      // )
      .rotateX(90)
      .translate([0, dim.z + dim.tampa.z, dim.y/2 + 10])
