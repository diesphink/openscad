...
"use strict"
  ...
  function conector_duto() {
    ...
    var dim_tubo = {r: 150/2, h: 60, parede: 2}
    var dim_grid = {qtd: 5, size: 3}
    var dim_base = {x: 5, y: 0, z: 5}
    var fn = 48
    // var dim_auxiliar = {r: 10, z: 10}

    var tubo = cylinder({r: dim_tubo.r, h: dim_tubo.h, center: [true, true, false], fn: fn})
      .subtract(cylinder({r: dim_tubo.r - dim_tubo.parede, h: dim_tubo.h, center: [true, true, false], fn: fn}))

    var base = cube({size: [dim_tubo.r * 2 + dim_base.x * 2, dim_tubo.r * 2 + dim_base.y * 2, dim_base.z], center: [1, 1, 0]})
      .subtract(cylinder({r: dim_tubo.r, h: dim_base.z, center: [true, true, false], fn: fn}))
    base = base
      .subtract(toolparts.makeChamferer(Y, [dim_base.z, 300, dim_base.z]).align(base, {center: [0, 1, 0], centerToBegin:[1, 0, 0], centerToEnd:[0, 0, 1]}))
      .subtract(toolparts.makeChamferer(Y, [dim_base.z, 300, dim_base.z]).align(base, {center: [0, 1, 0], centerToBegin:[0, 0, 0], centerToEnd:[1, 0, 1]}))
    ...
    var grid = cylinder({r: dim_tubo.r, h: dim_grid.size, center: [true, true, false], fn: fn})
...
main = function() {
  ...
  return conector_duto()//.splitZ(50)[1]
}
